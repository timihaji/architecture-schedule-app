    ...
    >
      ...
      {(() => {
        ...
        const mainCls = ['app-main',
          ...
          isGalleryWide ? 'is-gallery-wide' : '',
        ].filter(Boolean).join(' ');
        return (
      <main style={mainStyle} className={mainCls}>
        {view === 'library' && (
          <Library
            materials={materials}
            libraries={libraries}
...
function DesktopViewToggle() {
  ...
  return (
    <button onClick={toggle} className="desktop-view-toggle" style={{
...
// ─── Mobile Bottom Nav ───────────────────────────────────────────────────────
function MobileNavIcon({ d, size = 22 }) {
  return (
    <svg width={size} height={size} viewBox="0 0 24 24" fill="none"
    ...
    <nav className="mobile-bottom-nav" aria-label="Main navigation">
      {tabs.map(tab => (
        <button
          key={tab.key}
          type="button"
          className={'mobile-bottom-nav-btn' + (view === tab.key ? ' active' : '')}
          onClick={() => setView(tab.key)}
          aria-label={tab.label}
          aria-current={view === tab.key ? 'page' : undefined}
        >
          <MobileNavIcon d={tab.d} size={20} />
          <span>{tab.label}</span>
        </button>
      ))}
    </nav>
