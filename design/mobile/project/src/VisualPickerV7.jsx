...
(function () {
  ...
  const TEXTURE_MATERIALS = [
    // Timber
    { id: 'woodgrain',        cat: 'timber',  name: 'Woodgrain' },
    { id: 'veneer',           cat: 'timber',  name: 'Veneer' },
    { id: 'fluted',           cat: 'timber',  name: 'Fluted' },
    { id: 'castellation',     cat: 'timber',  name: 'Castellation' },
    { id: 'vj',               cat: 'timber',  name: 'VJ / Shiplap' },
    { id: 'wire-brushed',     cat: 'timber',  name: 'Wire-Brushed' },
    { id: 'charred',          cat: 'timber',  name: 'Charred' },
    // Stone (distinct patterns only)
    { id: 'marble',           cat: 'stone',   name: 'Marble' },
    { id: 'travertine',       cat: 'stone',   name: 'Travertine' },
    { id: 'terrazzo',         cat: 'stone',   name: 'Terrazzo' },
    // Concrete
    { id: 'board-formed',     cat: 'concrete',name: 'Board-Formed' },
    // Metal
    { id: 'brushed',          cat: 'metal',   name: 'Brushed' },
    { id: 'perforated',       cat: 'metal',   name: 'Perforated' },
    { id: 'mesh',             cat: 'metal',   name: 'Mesh' },
    { id: 'hammered',         cat: 'metal',   name: 'Hammered' },
    { id: 'corten',           cat: 'metal',   name: 'Corten' },
    // Glass
    { id: 'reeded-glass',     cat: 'glass',   name: 'Reeded' },
    { id: 'frosted-glass',    cat: 'glass',   name: 'Frosted' },
    { id: 'fluted-glass',     cat: 'glass',   name: 'Fluted' },
    // Textile
    { id: 'weave',            cat: 'textile', name: 'Weave' },
    { id: 'leather',          cat: 'textile', name: 'Leather' },
    { id: 'cane',             cat: 'textile', name: 'Cane / Rattan' },
    { id: 'boucle',           cat: 'textile', name: 'Boucle' },
    { id: 'quilted',          cat: 'textile', name: 'Quilted' },
    // Plaster
    { id: 'venetian-plaster', cat: 'plaster', name: 'Venetian' },
    { id: 'limewash',         cat: 'plaster', name: 'Limewash' },
    // Tile / Geometric patterns
    { id: 'ribbed',           cat: 'tile',    name: 'Ribbed' },
    { id: 'herringbone',      cat: 'tile',    name: 'Herringbone' },
    { id: 'scale',            cat: 'tile',    name: 'Scale' },
  ];

  const TEXTURE_DEFAULTS = {
    woodgrain:          { tone: '#C8A878', grain: '#7A5238' },
    veneer:             { tone: '#B89268', grain: '#6D4A24' },
    fluted:             { tone: '#A67D52', grain: '#6B4A28' },
    castellation:       { tone: '#A36948', grain: '#5A3319' },
    vj:                 { tone: '#EDE7DA', grain: '#8A7A68' },
    'wire-brushed':     { tone: '#C8A878', grain: '#8A6A4A' },
    charred:            { tone: '#1A1612', grain: '#3A3028' },
    marble:             { tone: '#ECE8DF', grain: '#8A8278' },
    travertine:         { tone: '#C8B49A', grain: '#8A7458' },
    terrazzo:           { tone: '#D8D0C0', grain: '#7A6858' },
    'board-formed':     { tone: '#9A9690', grain: '#6A6460' },
    brushed:            { tone: '#B8B5AE', grain: '#8A8780' },
    perforated:         { tone: '#8A8A88', grain: '#1E1E1C' },
    mesh:               { tone: '#8A8A88', grain: '#1E1E1C' },
    hammered:           { tone: '#5A5A5C', grain: '#3A3A3C' },
    corten:             { tone: '#A0522D', grain: '#7A3820' },
    'reeded-glass':     { tone: '#DCE8EC', grain: '#B0C8D0' },
    'frosted-glass':    { tone: '#EEF2F4', grain: '#C8D4D8' },
    'fluted-glass':     { tone: '#CCE0E8', grain: '#90B8C4' },
    weave:              { tone: '#CFC2A6', grain: '#9A8C6C' },
    leather:            { tone: '#8C6842', grain: '#5C4228' },
    cane:               { tone: '#D2BC88', grain: '#8A7248' },
    boucle:             { tone: '#E8E0D0', grain: '#A8A090' },
    quilted:            { tone: '#8C6842', grain: '#5C4228' },
    'venetian-plaster': { tone: '#E8E2D4', grain: '#C4BCA8' },
    limewash:           { tone: '#DCD4C4', grain: '#B4AA98' },
    ribbed:             { tone: '#A67D52', grain: '#6B4A28' },
    herringbone:        { tone: '#C8A878', grain: '#8A6A48' },
    scale:              { tone: '#7A9AAA', grain: '#4A6A7A' },
  };
  ...
  function migrateSwatchToV7(swatch) {
    ...
    if (s.kind === 'image' && s.src) {
      ...
      out.mode = 'texture';
      out.kind = 'texture';
      out.materialId = s.kind === 'stone' ? 'granite' : s.kind;
      out.grain = s.grain || s.vein || '#1a1a1a';
    } else {
      out.mode = 'colour';
  ...
  function VisualPickerV7({ swatch, setSwatch, materials }) {
    ...
    const selBg = React.useMemo(() => {
      ...
      // texture mode: SVG rendered as child; just provide tone as bg fallback
    ...
    function clearSelection() {
      ...
      else if (mode === 'texture') update({ materialId: null });
    }

    function pickTexture(materialId) {
      const defs = TEXTURE_DEFAULTS[materialId] || {};
      update({
        mode: 'texture',
        kind: 'texture',
        materialId,
        tone: sw.tone || defs.tone || '#c8a878',
        grain: sw.grain || defs.grain || '#7a5238',
      });
    }
      ...
      <div className="cf-block">
        ...
        <div className="cf-mode-row">
          <div className="cf-mode-toggle" role="tablist">
            {['colour','pattern','texture','image'].map(m => (
              <button key={m} type="button"
                className={'cf-mode-btn' + (mode === m ? ' on' : '')}
                onClick={() => setMode(m)}>
                {m === 'colour' ? 'Colour' : m === 'pattern' ? 'Pattern' : m === 'texture' ? 'Texture' : 'Image'}
              </button>
            ))}
          </div>
        </div>
        ...
        <div className="var-d"
          ...
          <div className={'var-d-sw' + (mode !== 'texture' ? ' ' + finishClass : '')}
            style={mode === 'texture' ? {} : selBg}>
            {mode === 'texture' && sw.materialId && window.renderTextureKind && (
              <div style={{ position: 'absolute', inset: 0, overflow: 'hidden' }}>
                {window.renderTextureKind(sw.materialId, {
                  tone: sw.tone || '#c8a878', grain: sw.grain || '#7a5238',
                  vein: sw.grain || '#7a5238', w: 120, h: 80,
                })}
              </div>
            )}
          </div>
          <div className="var-d-body">
            ...
            <div className="var-d-name">
              {mode === 'colour'
                ...
                : mode === 'pattern'
                  ...
                  : mode === 'texture'
                    ? (sw.materialId ? (TEXTURE_MATERIALS.find(m => m.id === sw.materialId)?.name || sw.materialId) : 'No texture')
            ...
            <div className="var-d-from">
              ...
              {mode === 'texture' && (
                <>
                  <span>{(sw.tone || '').toUpperCase()}</span>
                  {sw.grain && <> · <b>{(sw.grain || '').toUpperCase()}</b></>}
                </>
              )}
        ...
        {mode === 'texture' && (
          <TexturePanel sw={sw} update={update} pickTexture={pickTexture} />
        )}
        {mode === 'image' && (
  ...
  // ─── Texture panel ─────────────────────────────────────────────────────────
  function TexturePanel({ sw, update, pickTexture }) {
    const tone  = sw.tone  || '#c8a878';
    const grain = sw.grain || '#7a5238';
    const [cpTarget, setCpTarget] = React.useState(null); // 'tone' | 'grain' | null
    const [cpPalKey,  setCpPalKey]  = React.useState('timber');
    ...
    React.useEffect(() => {
      ...
      function onDown(e) {
        ...
        if (e.target.closest && e.target.closest('.cf-tex-color-sw')) return;
        setCpTarget(null);
      }
    ...
    function pickCpColor(hex) {
      ...
      if (cpTarget === 'tone')  update({ tone: HEX });
      else if (cpTarget === 'grain') update({ grain: HEX });
      setCpTarget(null);
    }

    const cats = ['all','timber','stone','metal','glass','textile','plaster','tile'];
    const list  = cat === 'all' ? TEXTURE_MATERIALS : TEXTURE_MATERIALS.filter(m => m.cat === cat);
      ...
      <>
        ...
        <div className="cf-pat-controls">
          <div className="cf-pat-color">
            <span className="cf-pat-lbl">Primary</span>
            <button type="button" className="cf-pat-color-sw cf-tex-color-sw" style={{ background: tone }}
              title="Choose primary colour"
              onClick={e => { e.stopPropagation(); setCpTarget(cpTarget === 'tone' ? null : 'tone'); }} />
            <span className="cf-pat-color-nm">{(findPaletteForHex(tone) || {}).name || tone}</span>
          </div>
          <div className="cf-pat-color">
            <span className="cf-pat-lbl">Secondary</span>
            <button type="button" className="cf-pat-color-sw cf-tex-color-sw" style={{ background: grain }}
              title="Choose secondary colour"
              onClick={e => { e.stopPropagation(); setCpTarget(cpTarget === 'grain' ? null : 'grain'); }} />
            <span className="cf-pat-color-nm">{(findPaletteForHex(grain) || {}).name || grain}</span>
          </div>
          <button type="button" className="cf-pat-swap" title="Swap Primary / Secondary"
            onClick={() => update({ tone: grain, grain: tone })}>
        ...
        {cpTarget && (
          <div ref={cpRef} className="cf-cp">
            <div className={'cf-cp-arrow' + (cpTarget === 'grain' ? ' bg' : '')} />
            <div className="cf-cp-head">
              <span className="cf-cp-eyebrow">{cpTarget === 'tone' ? 'Primary colour' : 'Secondary colour'}</span>
            ...
            <div className="cf-cp-grid">
              {(PALETTES[cpPalKey] || PALETTES.timber || PALETTES['warm-neutrals']).colors.map(([name, hex]) => {
                const cur = cpTarget === 'tone' ? tone : grain;
                return (
        ...
        {/* Material grid — tiles always shown in their natural default colours so
            the grid acts as a visual catalog; current picker colours only apply
            once you click a tile and work from there. */}
        <div className="cf-pat-grid">
          {list.map(mat => {
            const selected = sw.materialId === mat.id;
            const defs = TEXTURE_DEFAULTS[mat.id] || {};
            const tileTone  = defs.tone  || '#c8a878';
            const tileGrain = defs.grain || '#7a5238';
            return (
              <button key={mat.id} type="button"
                className={'cf-pat-tile' + (selected ? ' on' : '')}
                aria-selected={selected}
                onClick={() => pickTexture(mat.id)}>
                <span className="cf-pat-tile-sw" style={{ background: tileTone }}>
                  {window.renderTextureKind && window.renderTextureKind(mat.id, {
                    tone: tileTone, grain: tileGrain, vein: tileGrain, w: 120, h: 80,
                  })}
                </span>
                <span className="cf-pat-tile-nm">{mat.name}</span>
              </button>
            );
          })}
        </div>
  ...
  function SwatchMini({ swatch, style }) {
    ...
    let cls = '';
    if (sw.mode === 'texture' && sw.materialId) {
      const baseStyle = Object.assign({ position: 'relative', overflow: 'hidden', background: sw.tone || '#cdcfd2' }, style || {});
      return (
        <div className="sw-mini" style={baseStyle}>
          {window.renderTextureKind && (
            <div style={{ position: 'absolute', inset: 0, overflow: 'hidden' }}>
              {window.renderTextureKind(sw.materialId, {
                tone: sw.tone || '#c8a878', grain: sw.grain || '#7a5238',
                vein: sw.grain || '#7a5238', w: 120, h: 80,
