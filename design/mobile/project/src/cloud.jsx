...
(function () {
  ...
  // Pre-warm PostgREST so the first authenticated RPC doesn't pay the cold-
  // start tax. Low-traffic projects let PostgREST's schema cache age out;
  // hitting /rest/v1/ (returns the OpenAPI schema) wakes it without needing
  // auth. Fire-and-forget — even if it fails, the real RPC will still work.
  function prewarmPostgrest() {
    try {
  ...
  // Kick off the pre-warm as soon as cloud.jsx loads — gives PostgREST a head
  // start while the user types credentials or while session restore happens.
  prewarmPostgrest();

  // First-attempt timeout is generous (30s) since we expect pre-warm to cover
  // the common case but want one attempt to be able to ride out a cold start
  // by itself if pre-warm didn't fire (e.g. AuthGate mounted long after page
  // load). One retry at 20s as a safety net for transient errors. Ceiling
  // ~50s but the typical path now resolves in <1s post-prewarm.
