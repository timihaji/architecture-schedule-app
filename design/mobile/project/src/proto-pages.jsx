// proto-pages.jsx — Projects, Cost Schedule, Schedule, Settings
...
function ProjectEditor({ project, onSave, onClose, isNew }) {
  const [d, setD] = React.useState({ ...project });
  ...
  return (
    <FullDrawer onClose={onClose}>
      ...
      <div style={{ padding:'12px 16px', borderTop:'1px solid var(--rule)', display:'flex', gap:8, flexShrink:0, paddingBottom:'calc(12px + env(safe-area-inset-bottom,0px))' }}>
        ...
        <button onClick={() => { if(!d.name?.trim()) return; onSave(d); }} style={{ flex:1, background:'var(--ink)', color:'var(--paper)', border:'none', padding:'9px 18px', cursor:'pointer', fontFamily:'var(--font-sans)', fontSize:11, letterSpacing:'0.1em', textTransform:'uppercase', fontWeight:500, minHeight:44 }}>
  ...
  return (
    <div style={{ height:'100%', display:'flex', flexDirection:'column', background:'var(--paper)', overflow:'hidden' }}>
      ...
      <div style={{ padding:'14px 16px', borderBottom:'1px solid var(--rule)', flexShrink:0 }}>
        ...
        <button onClick={handleAdd} style={{ width:'100%', background:'var(--ink)', color:'var(--paper)', border:'none', padding:'10px', cursor:'pointer', fontFamily:'var(--font-sans)', fontSize:10.5, letterSpacing:'0.1em', textTransform:'uppercase', fontWeight:500, marginTop:12, minHeight:44, display:editMode?'block':'none' }}>＋ New project</button>
      </div>
      <div style={{ flex:1, overflowY:'auto', WebkitOverflowScrolling:'touch', paddingBottom:60 }}>
        {projects.length===0 && (
          <div style={{ padding:'48px 16px', textAlign:'center' }}>
            ...
            <button onClick={handleAdd} style={{ background:'none', border:'none', cursor:'pointer', fontFamily:'var(--font-sans)', fontSize:13, color:'var(--accent)', textDecoration:'underline' }}>＋ Create the first project</button>
          </div>
        )}
        {projects.map(p => (
          <div key={p.id} style={{ padding:'14px 16px', borderBottom:'1px solid var(--rule)', background:'var(--paper)' }}>
            ...
            <button onClick={() => setUi({ activeProjectId:p.id, view:'cost' })} style={{ background:'none', border:'none', padding:0, cursor:'pointer', textAlign:'left', width:'100%', display:'block' }}>
            ...
            <div style={{ display:'flex', gap:6, marginTop:10, flexWrap:'wrap' }}>
              ...
              <button onClick={() => setUi({ activeProjectId:p.id, view:'cost' })} style={{ background:'none', border:'1px solid var(--rule-2)', padding:'5px 12px', cursor:'pointer', fontFamily:'var(--font-sans)', fontSize:9, letterSpacing:'0.08em', textTransform:'uppercase', color:'var(--ink-4)', minHeight:36 }}>Cost →</button>
              <button onClick={() => setUi({ activeProjectId:p.id, view:'schedule' })} style={{ background:'none', border:'1px solid var(--rule-2)', padding:'5px 12px', cursor:'pointer', fontFamily:'var(--font-sans)', fontSize:9, letterSpacing:'0.08em', textTransform:'uppercase', color:'var(--ink-4)', minHeight:36 }}>Schedule →</button>
...
function AddRowSheet({ projectId, onClose }) {
  ...
  const [element, setEl]    = React.useState('');
  const [code, setCode]     = React.useState('');
  const [pickerOpen, setPO] = React.useState(false);
...
function CostPage() {
  ...
  React.useEffect(() => { if (!editMode) { exitSelect(); setAddOpen(false); setEditQty(null); setRowMenu(null); } }, [editMode]);
  ...
  if (!project) return (
    <div style={{ height:'100%', display:'flex', flexDirection:'column', background:'var(--paper)' }}>
      ...
      <div style={{ padding:'48px 16px', textAlign:'center' }}>
        ...
        <button onClick={() => setUi({ view:'projects' })} style={{ background:'none', border:'none', cursor:'pointer', fontFamily:'var(--font-sans)', fontSize:13, color:'var(--accent)', textDecoration:'underline' }}>Go to Projects →</button>
      </div>
    </div>
  );
        ...
        <div style={{ overflowX:'auto', WebkitOverflowScrolling:'touch' }}>
          <table style={{ width:'max-content', minWidth:'100%', borderCollapse:'collapse' }}>
            ...
            <tbody>
              ...
              {rows.length===0 && <tr><td colSpan={selectMode?8:7} style={{ padding:'32px 16px', textAlign:'center', fontFamily:'var(--font-serif)', fontStyle:'italic', fontSize:13.5, color:'var(--ink-4)' }}>No rows yet — tap Add row to begin.</td></tr>}
            </tbody>
          </table>
        </div>
      </div>
...
// SCHEDULE
// ════════════════════════════════════════════════════════════════
// In the Australian architecture context, a "schedule" is a specification
// document — one row per element/location pairing the spec material with
// its trade code, finish, supplier and notes. This is read+edit (not just
// read-only) and exposes a Detail toggle to flip between a compact list
// and a full mini-spec block per row.
...
function ScheduleRowSheet({ projectId, row, onClose }) {
  ...
  const [mat, setMat]      = React.useState(isEdit ? materials.find(m=>m.id===row.materialId) : null);
  const [qty, setQty]      = React.useState(String(initial.qty ?? ''));
  const [element, setEl]   = React.useState(initial.element || '');
...
// Spec field pairs shown in Detailed mode. Pulled from the material with
// graceful fall-throughs; rows without a material show only the row's own
// fields (element, qty, note).
      ...
      <div style={{ display:'flex', alignItems: detailed?'flex-start':'flex-start', gap:12 }}>
        ...
        <div style={{ flex:1, minWidth:0 }}>
          <div style={{ display:'flex', alignItems:'baseline', gap:8, justifyContent:'space-between' }}>
            <div className={detailed ? '' : 'clamp-2'} style={{ fontFamily:'var(--font-serif)', fontSize: detailed?16:14.5, color:'var(--ink)', lineHeight:1.22, minWidth:0, wordBreak:'break-word' }}>
...
function SchedulePage() {
  ...
  const editMode = ui.editMode;
  const [groupBy, setGroupBy]   = React.useState(() => { try { return localStorage.getItem('pm-sched-group')||'category'; } catch { return 'category'; } });
  const [detailed, setDetailed] = React.useState(() => { try { return localStorage.getItem('pm-sched-detail')==='1'; } catch { return false; } });
  const [sheet, setSheet]       = React.useState(null); // { kind:'add' } | { kind:'edit', row } | { kind:'actions', row }

  React.useEffect(() => { try { localStorage.setItem('pm-sched-group', groupBy); } catch {} }, [groupBy]);
  React.useEffect(() => { try { localStorage.setItem('pm-sched-detail', detailed?'1':'0'); } catch {} }, [detailed]);

  const project = (ui.activeProjectId ? projects.find(p=>p.id===ui.activeProjectId) : null) || projects[0];
  const rows = project ? ((schedules[project.id]||{}).rows||[]) : [];

  const cards = React.useMemo(() => rows.map(row => ({ row, mat: materials.find(m=>m.id===row.materialId) })), [rows, materials]);
    ...
    <div style={{ height:'100%', display:'flex', flexDirection:'column', background:'var(--paper)', overflow:'hidden', position:'relative' }}>
      ...
      <div style={{ padding:'12px 16px', borderBottom:'1px solid var(--rule)', flexShrink:0 }}>
        ...
        <div style={{ display:'flex', alignItems:'flex-end', justifyContent:'space-between', gap:8 }}>
          ...
          <button onClick={() => setUi({ view:'cost' })} style={{ background:'none', border:'none', cursor:'pointer', fontFamily:'var(--font-sans)', fontSize:10, letterSpacing:'0.08em', textTransform:'uppercase', color:'var(--ink-4)', flexShrink:0, minHeight:36 }}>Cost view →</button>
        </div>
        {projects.length>1 && (
...
function SettingsPage() {
  ...
  const sec = ui.settingsSection || 'appearance';
  const setSec = s => setUi({ settingsSection:s });
  const navSecs = [
    { key:'appearance', label:'Appearance' }, { key:'density', label:'Density' },
    { key:'typography', label:'Typography' }, { key:'library',    label:'Library'    },
    { key:'data',       label:'Data'       }, { key:'about',      label:'About'      },
  ];
  ...
  return (
    <div style={{ height:'100%', display:'flex', flexDirection:'column', background:'var(--paper)', overflow:'hidden' }}>
      ...
      <div style={{ flex:1, overflowY:'auto', WebkitOverflowScrolling:'touch', padding:'16px', paddingBottom:80 }}>
        ...
        {sec==='data' && <>
          <SettingRow label="Export archive" desc="Download all materials, projects, and schedules as JSON.">
            <button onClick={() => {
              const data = { materials: JSON.parse(localStorage.getItem('pm-materials')||'[]'), projects: JSON.parse(localStorage.getItem('pm-projects')||'[]'), schedules: JSON.parse(localStorage.getItem('pm-schedules')||'{}') };
              const blob = new Blob([JSON.stringify(data,null,2)],{type:'application/json'});
              const url = URL.createObjectURL(blob); const a = document.createElement('a'); a.href=url; a.download='archive.json'; a.click(); URL.revokeObjectURL(url);
            }} style={{ padding:'8px 16px', background:'transparent', border:'1px solid var(--ink)', fontFamily:'var(--font-sans)', fontSize:11.5, fontWeight:500, letterSpacing:'0.06em', textTransform:'uppercase', cursor:'pointer', color:'var(--ink)', minHeight:40 }}>Export JSON</button>
          </SettingRow>
          <SettingRow label="Restore defaults" desc="Wipe all data and reinstall the seed archive. Cannot be undone.">
            <button onClick={() => { if(window.confirm('Wipe all data and restore defaults?')) { ['pm-materials','pm-projects','pm-schedules','pm-libraries','pm-settings'].forEach(k=>localStorage.removeItem(k)); window.location.reload(); } }}
              style={{ padding:'8px 16px', background:'transparent', border:'1px solid rgba(160,69,69,0.4)', fontFamily:'var(--font-sans)', fontSize:11.5, fontWeight:500, letterSpacing:'0.06em', textTransform:'uppercase', cursor:'pointer', color:'#a04545', minHeight:40 }}>Restore defaults</button>
          </SettingRow>
        </>}

        {sec==='about' && (
          <div style={{ fontFamily:'var(--font-mono)', fontSize:11, color:'var(--ink-3)', lineHeight:1.8 }}>
            <div>Architecture Schedule App</div>
            <div>Mobile Prototype · Full interactive build</div>
            <div style={{ color:'var(--ink-4)', marginTop:8 }}>{materials.length} materials · {projects.length} projects</div>
            <div style={{ color:'var(--ink-4)' }}>State persists via localStorage.</div>
          </div>
        )}
      </div>
    </div>
...
Object.assign(window, { ProjectsPage, CostPage, SchedulePage, SettingsPage });
