// mobile-primitives.jsx — UI atoms shared across all mobile pages.
// Direct port of the prototype's proto-primitives.jsx. Pure UI, no coupling
// to data layer.
...
(function () {
  ...
  function SwatchBox({ tone, size = 32, style, onClick }) {
    return <div onClick={onClick} style={{ width: size, height: size, background: tone || '#ccc', flexShrink: 0, outline: '1px solid rgba(20,20,20,0.12)', ...(style || {}) }} />;
  }
  ...
  function SheetBg({ onClose, children }) {
    return (
      <div style={{ position: 'fixed', inset: 0, background: 'rgba(20,20,20,0.42)', zIndex: 400 }} onClick={onClose}>
        <div style={{ position: 'absolute', bottom: 0, left: 0, right: 0, background: 'var(--paper)', borderTop: '1px solid var(--ink)', maxHeight: '90vh', overflowY: 'auto', WebkitOverflowScrolling: 'touch', animation: 'mb-slide-up .22s cubic-bezier(.2,.8,.2,1)' }} onClick={e => e.stopPropagation()}>
          {children}
        </div>
      </div>
    );
  ...
  function FullDrawer({ children, onClose }) {
    return (
      <React.Fragment>
        ...
        <div style={{ position: 'fixed', inset: 0, background: 'var(--paper)', zIndex: 301, display: 'flex', flexDirection: 'column', overflow: 'hidden', animation: 'mb-slide-up .24s cubic-bezier(.2,.8,.2,1)' }}>
          {children}
        </div>
      </React.Fragment>
    );
    ...
    return (
      <div style={{ marginBottom: 12 }}>
        ...
        {multiline
          ...
          : <input type={type} value={value == null ? '' : value} onChange={e => onChange(e.target.value)} placeholder={placeholder || ''} style={base} />
        }
      </div>
    );
  }
  ...
  function TopNav() {
    const { settings, setSettings, ui, setUi } = window.useApp();
    const isDark = settings && settings.theme === 'dark';
    const editing = !!ui.editMode;
    return (
      <header className={'mb-topnav' + (editing ? ' mb-editing-ribbon' : '')}
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 9, minWidth: 0, overflow: 'hidden', flex: 1 }}>
          <div style={{ width: 18, height: 18, background: 'var(--ink)', flexShrink: 0, position: 'relative' }}>
            {editing && <span style={{ position: 'absolute', top: -3, right: -3, width: 8, height: 8, borderRadius: '50%', background: 'var(--accent)', border: '1.5px solid var(--paper)', animation: 'mb-pulse-dot 1.6s ease-in-out infinite' }} />}
          </div>
          <span style={{ fontFamily: 'var(--font-serif)', fontWeight: 400, fontSize: 15, letterSpacing: '-0.005em', whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis', color: 'var(--ink)' }}>Architecture Schedule</span>
  ...
  function ToastContainer() {
    const { toasts, dismissToast } = window.useApp();
      ...
      <div style={{ position: 'fixed', left: '50%', bottom: 72, transform: 'translateX(-50%)', zIndex: 500, display: 'flex', flexDirection: 'column-reverse', gap: 6, alignItems: 'center', pointerEvents: 'none', width: '100%', maxWidth: 'min(86vw, 360px)' }}>
        {toasts.map((t, i) => (
          <div key={t.id} onClick={() => dismissToast(t.id)} style={{
            ...
            animation: 'mb-toast-in .26s cubic-bezier(.2,.8,.2,1)',
  ...
  Object.assign(window, {
    MB_Serif: Serif, MB_Mono: Mono, MB_Eyebrow: Eyebrow,
    MB_SwatchBox: SwatchBox, MB_Checkbox: Checkbox, MB_StageBar: StageBar,
    MB_SheetBg: SheetBg, MB_FullDrawer: FullDrawer, MB_ConfirmSheet: ConfirmSheet,
    MB_SegBtn: SegBtn, MB_Field: Field,
    MB_TopNav: TopNav, MB_BottomNav: BottomNav, MB_ToastContainer: ToastContainer,
  });
})();
