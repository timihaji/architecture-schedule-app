// mobile-library.jsx — Library page, Material Editor, switcher, overlays.
// Port of proto-library.jsx, swapped to read/write via useApp() (adapter).

(function () {
  const { useState, useMemo, useEffect, useCallback } = React;
  const Serif    = () => window.MB_Serif;
  const useApp   = () => window.useApp();
  // Alias the primitives lazily — they're attached to window once
  // mobile-primitives.jsx has executed.
  function P() {
    return {
  ...
  function RegisterRow({ m, onEdit, selectMode, selected, onToggleSelect }) {
    const { Mono, SwatchBox, Checkbox, ConfirmSheet } = P();
  ...
  function MaterialEditor({ material, onSave, onClose, isNew }) {
    const { Eyebrow, FullDrawer, Field, ConfirmSheet } = P();
    ...
    return (
      <FullDrawer onClose={onClose}>
        ...
        <div style={{ padding: '12px 16px', borderTop: '1px solid var(--rule)', display: 'flex', gap: 6, background: 'var(--paper)', flexShrink: 0, paddingBottom: 'calc(12px + env(safe-area-inset-bottom,0px))' }}>
          ...
          <button onClick={() => { if (!draft.name || !draft.name.trim()) return; onSave(draft); }} style={{ flex: 1, background: 'var(--ink)', color: 'var(--paper)', border: 'none', padding: '9px 14px', cursor: 'pointer', fontFamily: 'var(--font-sans)', fontSize: 11, letterSpacing: '0.1em', textTransform: 'uppercase', fontWeight: 500, minHeight: 44 }}>
  ...
  function LibraryEditor({ library, onClose, isNew }) {
    const { Eyebrow, SheetBg, Field } = P();
    const { addLibrary, saveLibrary } = useApp();
    const [d, setD] = useState({ ...library });
  ...
  function LibrarySwitcher({ onClose }) {
    const { Mono, Eyebrow, SheetBg, ConfirmSheet } = P();
  ...
  function LibraryPickerSheet({ onPick, onClose }) {
    const { Eyebrow, SheetBg } = P();
  ...
  function LibraryPage() {
    const { Mono, Eyebrow, SegBtn, ConfirmSheet, TopNav } = P();
    ...
    const categories = useMemo(() => [...new Set(scoped.map(m => m.category).filter(Boolean))].sort(), [scoped]);
    ...
    function handlePick(categoryId) {
      setIsNew(true);
      setEditing({ id: '', name: '', code: '', category: categoryId, supplier: '', tone: '#c8c0b0', unit: 'm²', unitCost: null, leadTime: '', notes: '', libraryIds: [activeLibId !== 'all' ? activeLibId : (libraries[0]?.id || '')] });
      setKindOpen(false);
    }
    ...
    function exitSelectMode() { setSelectMode(false); setSel(new Set()); }
    ...
    useEffect(() => { if (!editMode) { exitSelectMode(); } }, [editMode]);
  ...
  Object.assign(window, { MB_LibraryPage: LibraryPage, MB_MaterialPickerOpen: null });
})();
